...
int attempt_collapse(
  ...
  if (feature_enable) {
    ...
    moved_tris.resize(neighbor0.size() - 2);
    for (auto i = 0; i < neighbor0.size();
         i++) {  // only distribute track from n0.
      auto f = old_fid[i];
    ...
    distributed_trackee = sub_refs.value();
    for (auto i = neighbor0.size(); i < old_fid.size(); i++) {
      auto f = old_fid[i];
      distributed_trackee.push_back(map_track[f]);
    }
  } else {
    std::set<int> combined_tracks;
    for (auto f : old_fid) {
      set_add_to(map_track[f], combined_tracks);
    }
    auto sub_refs = distort_check(base, mid, top, moved_tris, combined_tracks,
                                  refV, refF, option.distortion_bound,
                                  tree.num_freeze, option.dynamic_hashgrid);
...
int prism::local::wildcollapse_pass(PrismCage &pc, RemeshOptions &option) {
  ...
  while (!queue.empty()) {
    ...
    auto flag = [&pc, &option, &skip_flag, &V, &F, &n0, &n1, f0 = f, &f1, &u0,
                 &u1, &checker](int repeat_num) -> int {
      if (skip_flag[u0])  // skip if singularity or feature
        return 4;
      if (skip_flag[u1])
        repeat_num = 1;  // no shrinking on feature for now. TODO generic
                         // feature aware attempt.
      for (auto rp = 0; rp < repeat_num; rp++) {
        auto flag = prism::local_validity::attempt_collapse(
            pc, pc.track_ref, option, n0, n1, f0, f1, u0, u1, skip_flag[u1],
            checker);
    ...
    std::vector<int> new_fids_n01;
    std::vector<int> old_fids_n01;
    if (flag == 0 &&
        option.curve_checker.first
            .has_value()) {  // temporary copied code from validity_checks.
      std::vector<Vec3i> moved_tris;
      for (auto [f, e] : n0) {
        assert(pc.F[f][e] == u0);
        old_fids_n01.push_back(f);
        auto new_tris = pc.F[f];
        if (new_tris[0] == u1 || new_tris[1] == u1 || new_tris[2] == u1)
          continue;  // collapsed faces
        new_tris[e] = u1;
        moved_tris.emplace_back(new_tris);
        new_fids_n01.push_back(f);
      }
      for (auto [f, e] : n1) {
        auto new_tris = pc.F[f];
        old_fids_n01.push_back(f);
      ...
      std::sort(old_fids_n01.begin(), old_fids_n01.end());
      old_fids_n01.erase(std::unique(old_fids_n01.begin(), old_fids_n01.end()),
                         old_fids_n01.end());
      std::vector<int> shifts_n01 = prism::local_validity::triangle_shifts(
          moved_tris);  // this modifies moved tris in place
      if (option.curve_checker.first.has_value() &&
          !(std::any_cast<std::function<bool(
                const PrismCage &, const decltype(old_fids_n01) &,
                const decltype(moved_tris) &, decltype(local_cp) &)>>(
              option.curve_checker.first))(pc, old_fids_n01, moved_tris,
                                           local_cp)) {
        flag = 5;
      }
    }
    ...
    auto &[old_fids, new_fid, new_shifts, new_tracks] = checker;
    ...
    if (option.curve_checker.second.has_value())
      (std::any_cast<
          ...
          option.curve_checker.second))(old_fids_n01, new_fids_n01, local_cp);
    if (pc.top_grid != nullptr) {
      spdlog::trace("HashGrid Update");
      assert(old_fids.size() == new_fid.size() + 2);
      for (auto f : old_fids) {
